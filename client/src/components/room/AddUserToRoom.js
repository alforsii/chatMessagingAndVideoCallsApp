...
// otherUserId, roomId
const ADD_ROOM_USER_QUERY = gql`
  mutation($authorId: ID!, $otherUserId: ID!, $roomId: ID!) {
    addRoomUser(
      authorId: $authorId
...
export default function AddUserToRoom({ roomId }) {
  const [lgShow, setLgShow] = useState(false);
  const [message, setMessage] = useState("");
  const [foundUser, setFoundUser] = useState(null);
  const [searchInput, setSearchInput] = useState("");
  const [AddUser] = useMutation(ADD_ROOM_USER_QUERY);
      ...
      {(ctx) => {
        const handleAddUser = async (e) => {
          ...
          try {
            ...
            ctx.updateState({
              alertMessage: `${firstName} was added to the room: ${data.addRoomUser.roomName}!`,
              alertSuccess: true,
              alertMessageId: id,
            });
            setLgShow(false);
                    ...
                    {foundUser?.id && !message ? (
                      <>
                        <Alert variant={"primary"}>
                          {`We found ${foundUser.firstName} ${foundUser.lastName}. Would you like to add to the Room? `}
                          <Button
                            type="submit"
